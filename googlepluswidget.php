...
class gpw_widget_info extends WP_Widget {
	...
	global $debugtxt, $client;
		...
		if (!empty( $title ) ) $title = "Google Plus Posts";
  ...
  // The access token may have been updated lazily.
		...
		foreach($activities['items'] as $activity):
			...
			$text2 = strip_tags($texts[0]);
			$texts = explode('. ', $text2);
			$texts2 = explode('http', $texts[0]);
			$ptxt = $texts2[0];
			$reshare = "Reshared post from"; 
			if (substr_count($activity['title'],$reshare)==0) echo "<li><a href='".$activity['url']."'>".$ptxt."</a></li>";
		endforeach;
		echo "</ul></li>";
		echo $after_widget;		
	}
